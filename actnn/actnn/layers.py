...
from actnn.qscheme import QScheme as _QScheme
from actnn.qscheme import ACGC_QScheme as _ACGC_QScheme
from actnn.qscheme import ACGC_FixedQScheme as _ACGC_FixedQScheme
from actnn.qbnscheme import QBNScheme as _QBNScheme
...
def QScheme(*a, **kw):
    ...
    k = (bool(config.adaptive_conv_scheme), bool(config.acgc))
    Class = {
        (False, False): None,               # Not Adaptive or ACGC
        (True,  False): _QScheme,           # ActNN
        (False, True):  _ACGC_FixedQScheme, # ACGC w fixed bits
        (True,  True):  _ACGC_QScheme,      # ACGC w auto bits
    }[k]
    
    name = Class.__name__ if Class else 'None'
    # print('{} {}({}, ...)'.format(config.activation_compression_bits, name, layer))
    return Class(*a, **kw) if Class else None
    
    
def QBNScheme(*a, **kw):
    layer = a[0]
    
    k = (bool(config.adaptive_bn_scheme), bool(config.acgc))
    Class = {
        (False, False): None,               # Not Adaptive or ACGC
        (True,  False): _QBNScheme,         # ActNN
        (False, True):  _ACGC_FixedQScheme, # ACGC w fixed bits
        (True,  True):  _ACGC_QScheme,      # ACGC w auto bits
    }[k]
...
class QBatchNorm1d(nn.BatchNorm1d):
    def __init__(self, num_features, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True, group=0):
        ...
        self.scheme = QBNScheme(self, group=group)
