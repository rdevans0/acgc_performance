...
from actnn.utils import to_cupy, to_torch
...
from actnn.fixpoint import fixpoint_compress, fixpoint_decompress
from actnn.zvc import zvc_compress, zvc_decompress
...
def nan_check(**kw):
    ...
    for k, x in kw.items():
        ...
        if isinstance(x, torch.Tensor):
            ...
            bad = cupy.isnan(x).any() or cupy.isinf(x).any()

        if bad:
            raise Exception('Nan check failed for {} {}'.format(k, x.shape))
...
def dequantize_and_unpack(data, shape, bits, scale, mn):
    if config.simulate:
        ...
        num_features = (num_features + (group_size -
        ...
        data = unpack_func(data, bits, scale, mn, N,
                           num_features // group_size, group_size)
    return data
...
@profile
def quantize_activation(input, scheme, layer=''):
    ...
    elif config.acgc_quant:
        x = to_cupy(input)
        assert scheme is not None, 'Always expects a scheme'
        
        err_or_bits, f, signed, per_channel = scheme.compute_quantization_bits(x)
        sign_bit = 1 if signed else 0
        
        if scheme.auto:
            err = err_or_bits
            bits = - cupy.log2(err) - cupy.log2(f) - 1 + sign_bit
            bits = int(bits.mean())
        else:
            bits = err_or_bits

        # print('Quantize with Quant')

        # if layer in ('linear',):
        #     bits = 12
        #     print(abs(x).max())
        #     print(abs(x*f).max())
        #     #np.savez('1_x_pre_c.npz', x=x.get(), x_f=(x*f).get())
        
        x_f = x*f
        del x
        
        zvc = config.zvc and not signed and bits > 2
        if zvc:
            x_f, nzvs = zvc_compress(x_f, impl='mask')
            nnz = x_f.size
            
        # Compress
        quant = fixpoint_compress(
            x_f, bits, scale=None, signed=signed, impl='cupy')
        
        
        #print(x.shape, np.array2string(quant.get()[
        #      :100], max_line_width=255, threshold=100))

        # if layer in ('linear',):
        #     np.savez('2_x_post_c.npz', x=x.get(), x_f=(x*f).get())
        #     np.savez('2_q_post_c.npz', quant=quant.get())

        #q_scale = to_torch(f)
        #if input.dtype == torch.float32:
        #    q_scale = q_scale.to(torch.bfloat16)
        # if layer in ('linear',):
        #     cupy.cuda.get_current_stream().synchronize()

        if zvc:
            return (quant, nzvs, nnz), bits, f, signed
        else:
            return quant, bits, f, signed


    else:
        # print('Quantize with ActNN')
        if scheme:
            input_groups, q_bits, q_min, mx = scheme.compute_quantization_bits(
                input)
        else:
            input_groups, q_bits, q_min, mx = no_scheme_compute_quantization_bits(
                input)
...
@profile
def dequantize_activation(quantized, q_input_shape, layer=''):
    if not config.compress_activation:
        ret = quantized[0]
        if config.swap:
        ...
        #print('Dequantize with Quant')
        q_, bits, f, signed = quantized
        zvc = config.zvc and not signed and bits > 2
        ...
        #print(q_input_shape, np.array2string(
        #    quant.get()[:100], max_line_width=255, threshold=100))
        ...
        # if layer in ('linear',):
        #     np.savez('3_x_pre_d.npz', x=x_.get(), x_f=(x_*f).get())
        #     np.savez('3_q_pre_d.npz', quant=quant.get())
        
        x = fixpoint_decompress(quant, bits, scale=None,
                                signed=signed, impl='cupy')
        
        if zvc:
            x = zvc_decompress(x[:nnz], nzvs, q_input_shape, impl='mask')
        
        
        # if layer in ('linear',):
        #     np.savez('4_x_post_d.npz', x_f=x.get())

        if config.debug_nan and cupy.isnan(x).any():
            raise Exception('Nans in decompressed output!')
        
        x = x.reshape(q_input_shape)

        if config.debug_nan and cupy.isnan(x).any():
            raise Exception('Nans in decompressed output!')

        # if config.debug_acgc:
        #     print(abs(x_*f).max())
        #     print(abs(x).max())
        #     print(np.array2string(abs(cupy.around(
        #         x_*f, 5))[:10, :10].get(), max_line_width=250, formatter={'all': '{:10f}'.format}))
        #     print(np.array2string(abs(cupy.around(x, 5))[:10, :10].get(
        #     ), max_line_width=250, formatter={'all': '{:10f}'.format}))
        #     print('--')
        #     print(np.array2string(abs(cupy.around(x-x_*f, 5))
        #                           [:10, :10].get(), max_line_width=250, formatter={'all': '{:10f}'.format}))

        #     max_err = 2**-bits if not signed else 2**(-bits+1)
        #     err = abs(x - x_*f)[(x_*f >= -1) * (x_*f <= 1)].max()
        #     if err > max_err:
        #         sat = (abs(x - x_*f)[(x_*f >= -1) *
        #                              (x_*f <= 1)] > max_err).sum()
        #         sat = float(sat)
        #         raise Exception('Error is too high '
        #                         + '\n Actual {}   Max {}'.format(err, max_err)
        #                         + '\n Good {}, Bad {:.1f}%'.format(sat, sat/x.size*100))
        ...
        if config.debug_nan and torch.isnan(input).any():
            raise Exception('Nans in decompressed output!')
...
class convnd(Function):
    ...
    def run_backward(n, ctx, grad_output, bias_reduce_dims, aug):
        ...
        if ctx.scheme and ctx.scheme.ty == 'actnn':
        ...
        if config.debug_memory_op_backward:
            ...
            print("========== conv%dd backward %d ==========" %
                  (n, conv2d_layer_ct))
            get_memory_usage(True)
            conv2d_layer_ct += 1
            print("WS: %.2f MB" % (compute_tensor_bytes(
                [grad_output, input, input]) / 1024 ** 2))
        ...
        if use_pipeline:
            micro_batch_size = (
                ws_mem + config.pipeline_threshold) // config.pipeline_threshold
            raw_input = input
        ...
        if ctx.scheme and ctx.scheme.ty == 'acgc':
            ctx.scheme.update_grads(
                grad_output=grad_output, grad_weight=grad_weight)

        nan_check(grad_output=grad_output, grad_weight=grad_weight)
        if ctx.scheme:
...
class conv_transposend(Function):
    ...
    def run_forward(n, forward_op, ctx, input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1, scheme=None):
        ...
        ctx.scheme = scheme
        ctx.saved = quantized, weight, bias
        ctx.other_args = (input.shape, stride, padding,
        ...
        if config.debug_memory_op_forward:
            global conv2d_layer_ct, total_act_mem
            print("========== conv%dd_transpose forward %d ==========" %
                  (n, conv2d_layer_ct))
            get_memory_usage(True)
            conv2d_layer_ct += 1
            total_act_mem += compute_tensor_bytes(quantized)
    ...
    def run_backward(n, ctx, grad_output, bias_reduce_dims, aug):
        ...
        if config.debug_memory_op_backward:
            global conv2d_layer_ct
            print("========== conv%dd_transpose backward %d ==========" %
                  (n, conv2d_layer_ct))
            get_memory_usage(True)
            conv2d_layer_ct += 1
            print("WS: %.2f MB" % (compute_tensor_bytes(
...
class linear(Function):
    ...
    def forward(ctx, input, weight, bias=None, scheme=None):
        quantized = quantize_activation(input, scheme, layer='linear')
    ...
    def backward(ctx, grad_output):
        ...
        input = dequantize_activation(quantized, q_input_shape, layer='linear')
        del quantized, ctx.saved
        ...
        nan_check(grad_output=grad_output,
                  grad_weight=grad_weight, grad_bias=grad_bias)
        if ctx.scheme:
            ctx.scheme.if_allocate_perlayer()
...
class batch_norm(Function):
    ...
    def forward(ctx, input, running_mean, running_var, weight, bias,
        ...
        ctx.scheme = scheme
        ctx.other_args = input.shape
        ctx.saved = (quantized, weight, running_mean, running_var,
                     save_mean, save_var, training, eps, reserve)
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.scheme and ctx.scheme.ty == 'acgc':
            ctx.scheme.update_grads(
                ...
                grad_output=grad_output, grad_weight=grad_weight, 
                weight=weight, save_var=save_var)
...
class adaptive_avg_pool2d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        repeat_size = [int(x / y)
                       for x, y in zip(input_shape, grad_output.shape)]
