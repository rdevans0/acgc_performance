...
parser.add_argument('--acgc', type=str, default=None, choices=['quant','quantz','cusz'],
                    help='ACGC algorithm to use')
parser.add_argument('--recalc_interval', type=int, default=100,
                    help='Recalculates ACGC bitwidths every X iterations')
parser.add_argument('--bits', type=int, default=4, 
                    help='Number of ACGC bits, or epsilon setting')
...
def main():
    ...
    actnn.set_optimization_level(args.alg, **vars(args))
    ...
    if args.multiprocessing_distributed:
        ...
        raise Exception('Say NO to threads!')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.alg in ['swap'] or args.alg.startswith('actnn') or args.alg.startswith('acgc'):
        print("=> convert model")
        model = QModule(model)
        model.cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        ret = train(train_loader, model, criterion, optimizer, epoch, args)
